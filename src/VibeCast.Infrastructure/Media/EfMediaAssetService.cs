    ...
    ILogger<EfMediaAssetService> logger) : IMediaAssetService
{
    ...
    public async Task<IReadOnlyList<MediaAssetSummary>> ListKnowledgeSourcesAsync(string ownerId, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(ownerId))
        {
            throw new ArgumentException(
    ...
    public async Task SetKnowledgeSourceAsync(Guid mediaAssetId, bool isKnowledgeSource, string ownerId, CancellationToken cancellationToken = default)
    {
        ...
        if (asset is null)
        {
            throw new Exception("The selected media asset was not found.");
        }

        if (!MediaAssetHelpers.IsSupportedDocumentType(asset.ContentType))
        {
            throw new Exception("Only PDF and TXT documents can currently be used as knowledge sources.");
        }
